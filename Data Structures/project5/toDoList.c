...
{
    /*FIXME: write this*/
	if( ((struct Task *)left)->priority < ((struct Task *)right)->priority ){ return -1;}
	if( ((struct Task *)left)->priority > ((struct Task *)right)->priority ){ return 1;}
...
void print_type(TYPE val)
{
  ...
  printf("Task: %s  Priority: %d", v->description, v->priority);
}
...
{
  /*FIXME: Write this */
  struct Task* new_task = malloc(sizeof(TYPE));
  int *temp = malloc(sizeof(int));
  *temp = priority;
  ((struct Task *)new_task)->priority = *temp;
  strcpy(((struct Task *)new_task)->description, desc); 
