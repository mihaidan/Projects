...
{
  /* FIXME: Write This */
  addDynArr(heap, node);//adds to end -- now need to adjust position
  int temp_pos = sizeDynArr(heap) - 1;
  int temp_parent = sizeDynArr(heap) - 1;
  while((temp_pos - 1)/2 >= 0)
  {
	temp_parent = (temp_pos - 1)/2;
	if( (*compare)(getDynArr(heap, temp_pos), getDynArr(heap, temp_parent) ) == -1 )
	{
		swapDynArr(heap, temp_pos, temp_parent);
    }
	else { break;}
	temp_pos = temp_parent;
  }
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  ...
  if(rc < max)
  {
	smaller_child = _smallerIndexHeap(heap, lc, rc, compare);
	if( (*compare)(getDynArr(heap, pos), getDynArr(heap, smaller_child) ) == 1 )
	{
		swapDynArr(heap, pos, smaller_child);
		_adjustHeap(heap, max, smaller_child, compare);
	}
  }
  else if(lc < max)
  {
	if( (*compare)(getDynArr(heap, pos), getDynArr(heap, lc) ) == 1 )
	{
		swapDynArr(heap, pos, lc);
		_adjustHeap(heap, max, lc, compare);
	}
  }
}
...
void _buildHeap(DynArr *heap, comparator compare)
{
  ...
  int i;
  for (i = max/2 - 1; i >= 0; i--) { _adjustHeap(heap, max, i, compare);}
}
/*
