...
* Author: Mihai Dan
* Date 12/1/2015
* Compile: gcc -std=c99 -o otp_enc otp_enc.c
**********************************************/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
* Description: Captures the text from the text and key files,
			   stores them in and array, and sends them to the
			   daemon to be processed. The cipher text will be
...
int main(int argc, char *argv[]){
	...
	if(argc != 4){
		printf("Please follow the format \"otp_enc <plaintext> <key> <port>\"\n");
		exit(1);
	}
