...
int main (int argc, const char * argv[]) {
    ...
    if(fileptr){
		...
		printf("\nERROR: Invalid input, file is protected or does not exist.\n\n"); 
		exit(1);
	}
	...
	char *nextWord;
		...
		//loop through file until getWord returns NULL
	while(nullCheck == 0)
		{
			nextWord = getWord(fileptr);
		
			//check if getWord returned NULL
			if(!nextWord){nullCheck = 1; break;}
			
			//check if table already contains nextWord
			if(containsKey (hashTable, nextWord))
			{
				int *temp = (int*)atMap(hashTable, nextWord);
				removeKey(hashTable, nextWord);
				insertMap(hashTable, nextWord, (int*)(temp + 1));
			}
			else{ insertMap(hashTable, nextWord, (int*)(1));}
